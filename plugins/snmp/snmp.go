...
func (h *Host) SNMPGet(acc plugins.Accumulator) error {
	// Get snmp client
...
func (h *Host) SNMPBulk(acc plugins.Accumulator) error {
	// Get snmp client
	snmpClient, err := h.GetSNMPClient()
	if err != nil {
		return err
...
func (h *Host) HandleResponse(oids map[string]Data, result *gosnmp.SnmpPacket, acc plugins.Accumulator) (string, error) {
	var lastOid string
...
func init() {
	plugins.Add("snmp", func() plugins.Plugin {
		return &Snmp{}
	})
}
